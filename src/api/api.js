...
const animeEpisodeHandler = async (id) => {
  ...
  $('div#wrapper_bg').each((index, element) => {
    ...
    $element.find('div.anime_muti_link ul li').each((j, el) => {
      ...
      if (iframe.includes('streaming.php')) {
        iframeUrl = iframe;
      }
    })
    ...
    // promises.push(videoUrlHandler(iframeUrl).then(extra2 => animeContentHandler(animeId).then(extra => ({
    //   img: extra[0] ? extra[0].img : null,
    //   synopsis: extra[0] ? extra[0].synopsis : null,
    //   genres: extra[0] ? extra[0].genres : null,
    //   category: category ? category : null,
...
const videoUrlHandler = async (url) => {
  ...
  page.on('request', request => {
    if (request._url.includes('encrypt-ajax')) {
      console.log('REQUEST ' + request._url);
    }
    request.continue();
  });
  page.on('response', async (response) => {
    if (response._url.includes('encrypt-ajax')) {
      var json = await response.json();
      videoUrl = json.source_bk[0].file;
      console.log('RESPONSE ' + json.source_bk[0].file);
    }
  });
...
const animeContentHandler = async (id) => {
  ...
  const check_zero_episode_axios = await axios.get(`${url.BASE_URL}category/${id.split('/')[2]}`);
